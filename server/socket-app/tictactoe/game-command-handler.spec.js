let Handler = require('./game-command-handler');

describe("Game command handler", function () {

    let handler;

    let commandRouter, eventRouter;

    let MessageRouter = require('client/src/common/framework/message-router');

    let executedCommand;
    let aggregate = function(events){
        return{
    ...
    it('should route resulting events to event router with all relevant information',function(){
        ...
        expect(resultingEvents[0].commandId).toEqual('thisIsTheCommand');
